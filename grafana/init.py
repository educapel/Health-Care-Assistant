...
def get_api_key():
    """Use existing API key from environment or create new service account token"""
    if GRAFANA_API_KEY:
        ...
        if response.status_code == 200:
            ...
            print("Existing API key is invalid, need to create new one")

    print("No valid API key found. Please run debug_grafana.py to create one first.")
    return None
...
def main():
    api_key = get_api_key()  # Changed from create_api_key()
    if not api_key:
        print("API key validation failed")
        print("Run: python debug_grafana.py to create a new token")
        return
